<#
## Function to  Compare SQL Server Table schemas
Return True if it mataches schema 
otherwise false
#
1. Columns: In this section we check the number of columns, the name of the columns, 
the data types for every column, if is part of a primary key, if is part of a foreign key, if is a computed column and so on.

2. Checks: In this section all checks defined in a table are compared. 
As we explained before, the name doesnâ€™t matter, the only thing that matters is the check definition text and the columns involved.

3. Indexes: In this sections all the indexes are compared. 
The index name doesn't make a difference, we only check for index type, columns and so on.
#>
